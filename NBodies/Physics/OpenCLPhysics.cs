...
namespace NBodies.Physics
{
    public class OpenCLPhysics : IPhysicsCalc, IDisposable
    {
        ...
        public void Init()
        {
            ...
            _cellMapKernel = _program.CreateKernel("MapMesh");
        ...
        private void BuildMeshGPU(int cellSizeExp)
        {
            ...
            _cellMapKernel.SetValueArgument(6, 2); // Set the mort offset to 2 in order to skip the Y elements of the long2 type.
            ...
            for (int level = 1; level <= _levels; level++)
            {
                ...
                _cellMapKernel.SetLocalArgument(4, SIZEOFINT * _threadsPerBlock);
                _cellMapKernel.SetValueArgument(5, _threadsPerBlock);
                _cellMapKernel.SetValueArgument(6, 1); // Using long type here. Set the offset to 1 so no elements are skipped.
                _queue.Execute(_cellMapKernel, null, new long[] { blocks * _threadsPerBlock }, new long[] { _threadsPerBlock }, null);
