...
namespace NBodies.Physics
{
    public class CUDADouble : IPhysicsCalc
    {
        ...
        public CUDADouble(int gpuIdx, int threadsperblock)
        {
            if (gpuIdx != -1)
                gpuIndex = gpuIdx;
            ...
            for (int b = 0; b < len; b++)
            {
                ...
                if (iBody.Visible == 1)
                {
                    ...
                    if (iBody.UID != body.UID)
                    {
                        ...
                        body.ForceTot += force;
                        body.ForceX += (float)(force * distX / distSqrt);
                        body.ForceY += (float)(force * distY / distSqrt);
                                ...
                                if (DistSqrt > 0)
                                {
                                    ...
                                    if (outBody.InRoche == 1 & inBody.InRoche == 1)
                                    {
                                        if (outBody.Density > 0 && inBody.Density > 0)
                                        {
                                            ...
                                            float gradX = (float)(DistX * gradFactor);
                                            float gradY = (float)(DistY * gradFactor);
                                    ...
                                    else
                                    {
                                        ...
                                        M1 = outBody.Mass;
                                        M2 = inBody.Mass;
                                        vecX = (float)DistX * 0.5f;
                                        vecY = (float)DistY * 0.5f;
